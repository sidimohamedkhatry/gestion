        ...
        <section class="content">
            <div class="box box-primary">
                ...
                <div class="box-header with-border" style="margin-bottom:10px ">
                    <h3 class="box-title"><i class="fa fa-users"></i> @lang('site.clients')
                        <small><mark>{{ $clients->count() }}</mark></small></h3>
                    <form action="{{ route('dashboard.clients.index') }}" method="GET" style="margin-top:10px;"
                        autocomplete="off">
                        <div class="row">
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                <button type="submit" class="btn btn-primary btn-sm">
                                 ...
                                 @if (auth()->user()->hasPermission('clients-create'))
                                    <a href="{{ route('dashboard.clients.create') }}" class="btn btn-primary btn-sm"><i
                                            class="fa fa-plus"></i> @lang('site.add_client') </a>
                        ...
                        <table class="table table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <td>@lang('site.orders')</td>
                                     <td>@lang('site.orders')</td>                                   
                                    <td>@lang('site.add_order')</td>
                            ...
                            <tbody>
                             @if ($clients->count() > 0)
                                @foreach ($clients as $index => $client)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td><img src="{{ $client->image_path }}" alt="" style="width: 50px"></td>
                                        <td>{{ $client->name }}</td>
                                        <td>{{ $client->phone }}</td>
                                        <td>{{ $client->address }}</td>

                                        <td>{{ $client->orders->count() }}</td>
                                     <td><a href="{{ route('dashboard.orders.index',['client_id' => $client->id]) }}" class="btn btn-info btn-sm">@lang('site.orders')</a></td>

                                        <td>
                                            @if (auth()->user()->hasPermission('orders-create'))
                                                <a class="btn btn-primary btn-sm"
                                                    href="{{ route('dashboard.clients.orders.create', $client->id) }}">@lang('site.add_order')</a>
                                            @else
                                                <a class="btn btn-primary btn-sm disabled"
                                                    href="#">@lang('site.add_order')</a>
                                            @endif
